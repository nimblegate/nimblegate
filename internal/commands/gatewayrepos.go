...
func reposAddFormInner(opts reposPageOpts) string {
	return fmt.Sprintf(`<form hx-post="/policy/repo/add" hx-headers='{"X-CSRF-Token":"%s"}' hx-encoding="application/x-www-form-urlencoded" hx-target="body" hx-swap="outerHTML"><label>Name <input type="text" name="name" required pattern="[a-z0-9][a-z0-9._\-]*" placeholder="my-repo" title="Lowercase letters, numbers, dots (.), hyphens (-) and underscores (_). Must start with a letter or number. No spaces, no uppercase."></label><p class="sub">Lowercase letters, numbers, dots, hyphens, underscores. Must start with a letter or number.</p><label>Upstream URL <input type="text" name="upstream" required placeholder="https://github.com/owner/repo.git"></label><p class="sub">The repo's <b>HTTPS</b> clone URL (e.g. <code>https://github.com/owner/repo.git</code>) - not the <code>git@…</code> SSH form. The token below authenticates it; works for private repos too.</p><label>Upstream credential <input type="password" name="upstream_credential" autocomplete="new-password" placeholder="PAT / deploy token (optional, stored mode 0600, never logged)"></label><label>Protected refs <input type="text" name="protected_refs" value="refs/heads/*" placeholder="refs/heads/* refs/heads/main refs/heads/release/*" title="space-separated full ref names; bare branch names like &#34;main&#34; are auto-prefixed with refs/heads/. A trailing /* is recursive: &#34;refs/heads/*&#34; gates every branch at any depth, including agent/x and feature/x."></label><p class="sub">Default <code>refs/heads/*</code> gates <b>every</b> branch (required for the auto-PR loop). Gates push content only; branch deletion is separate, so main stays protected while feature branches remain deletable.</p><fieldset class="gw-status-fieldset"><legend>Status</legend><label><input type="checkbox" name="enabled" value="1" checked> enabled</label><label><input type="checkbox" name="observe" value="1"> observe-only</label></fieldset><p class="gw-add-note">The <b>Core</b> kit (18 catastrophic-prevention frames) is applied automatically. After registering, refine the selection in the Frame selection section at the bottom: apply additional kits (Web app, CF Pages, CF Workers, Security strict) or tick individual frames.</p><button type="submit">Register repo</button><p class="sub">First push will auto-scan and recommend more.</p></form>`,
		htmlEsc(opts.CSRFToken))
}
...
func renderRepoTable(w io.Writer, rows []repoRow, opts reposPageOpts) {
	...
	for _, row := range rows {
		...
		// Edit-repo-settings (upstream URL + protected refs) as a collapsible row
...
// repoSettingsForm renders the collapsible Edit-repo-settings form for one repo.
